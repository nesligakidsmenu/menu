...
function dinamikAnaKategorileriGoster(kategoriler) {

    try {
    const alan = document.getElementById("anaKategoriListesi");
    if (!alan || !Array.isArray(kategoriler)) return;
...
// Dinamik ana kategorileri ekle
const kategoriCevap = await fetch("categories.json");
const kategoriler = await kategoriCevap.json();
    kategoriler.forEach((kategoriBilgisi) => {

const alan = document.getElementById("anaKategoriListesi");
        const kategoriKodu = String(kategoriBilgisi?.id || "").trim();
        const kategoriAdi = String(kategoriBilgisi?.name || kategoriKodu).trim();

if (alan) {
        if (!kategoriKodu || !kategoriAdi || mevcutKategoriKodlari.has(kategoriKodu)) return;

    const mevcutlar = [...alan.querySelectorAll("a.category")]
        .map(a => new URL(a.href).searchParams.get("kat"));
    ...
    kategoriler.forEach(k => {
        ...
        alan.insertAdjacentHTML("beforeend", `
            <a href="kategori.html?kat=${k.id}" class="category">
                <div class="icon">
                    <img src="./assets/icons/default.webp" alt="${k.name}">
                </div>
                <span>${k.name}</span>
            </a>
        `);
...
const kategoriCevap = await fetch("categories.json");
const kategoriler = await kategoriCevap.json();
dinamikAnaKategorileriGoster(kategoriler);
