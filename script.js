...
function urunleriSirala(urunler) {
    ...
    return urunler
        .map((urun, index) => ({
            urun,
            index,
            order: Number.isFinite(Number(urun?.order)) ? Number(urun.order) : index + 1
        }))
        .sort((ilk, ikinci) => ilk.order - ikinci.order || ilk.index - ikinci.index)
        .map(({ urun }) => urun);
}

function kategoriKodunuGuzellestir(kategoriKodu) {

    return String(kategoriKodu || "")
        .replace(/[-_]/g, " ")
        .replace(/\b\w/g, (harf) => harf.toLocaleUpperCase("tr-TR"));
}

function dinamikAnaKategorileriGoster(kategoriler, urunler) {

    const alan = document.getElementById("anaKategoriListesi");
    if (!alan) return;

    const kayitliKategoriler = Array.isArray(kategoriler) ? kategoriler : [];
    const urunKategorileri = Array.isArray(urunler)
        ? urunler
            .map((urun) => String(urun?.anaKategori || "").trim())
            .filter(Boolean)
            .map((kategoriKodu) => ({
                id: kategoriKodu,
                name: kategoriKodunuGuzellestir(kategoriKodu)
            }))
        : [];
    const tumKategoriler = [...kayitliKategoriler, ...urunKategorileri];

    const mevcutKategoriKodlari = new Set(
        [...alan.querySelectorAll("a.category")]
            .map((baglanti) => new URL(baglanti.href).searchParams.get("kat"))
            .filter(Boolean)
    );

    tumKategoriler.forEach((kategoriBilgisi) => {

        const kategoriKodu = String(kategoriBilgisi?.id || "").trim();
        const kategoriAdi = String(kategoriBilgisi?.name || kategoriKodunuGuzellestir(kategoriKodu)).trim();

        if (!kategoriKodu || !kategoriAdi || mevcutKategoriKodlari.has(kategoriKodu)) return;

        const kart = document.createElement("a");
        kart.className = "category";
        kart.href = `kategori.html?kat=${encodeURIComponent(kategoriKodu)}`;

        const ikon = document.createElement("div");
        ikon.className = "icon";
        const resim = document.createElement("img");
        resim.src = "./assets/icons/default.webp";
        resim.alt = kategoriAdi;
        ikon.append(resim);

        const ad = document.createElement("span");
        ad.textContent = kategoriAdi;
        kart.append(ikon, ad);
        alan.append(kart);
        mevcutKategoriKodlari.add(kategoriKodu);
    });
}

function kategoriAltKategorileriniGetir(veriler, kategoriler, kategoriKodu) {

    const kayitliKategori = Array.isArray(kategoriler)
        ? kategoriler.find((item) => String(item?.id || "") === kategoriKodu)
        : null;
    const kayitliAltKategoriler = Array.isArray(kayitliKategori?.subCategories)
        ? kayitliKategori.subCategories
        : [];
    const urunAltKategorileri = veriler
        .filter((urun) => urun.anaKategori === kategoriKodu)
        .map((urun) => urun.altKategori);

    return [...new Set([...kayitliAltKategoriler, ...urunAltKategorileri]
        .map((altKategoriKodu) => String(altKategoriKodu || "").trim())
        .filter(Boolean))];
}
...
const urunler = urunleriSirala(await cevap.json());

const kategoriCevap = await fetch("categories.json");
const kategoriler = await kategoriCevap.json();
dinamikAnaKategorileriGoster(kategoriler, urunler);

document.getElementById("arama") &&
    aramaKutusu(urunler);

document.getElementById("kategoriListesi") &&
    kategoriSayfasi(urunler, kategoriler);
...
function kategoriSayfasi(veriler, kategoriler = []) {
...
if (!altKategori && !DIREKT_URUN_KATEGORILERI.includes(kategori)){
        const altlar = kategoriAltKategorileriniGetir(veriler, kategoriler, kategori);

        if (!altlar.length) {
            alan.innerHTML = "<p>Bu kategoride henüz alt kategori bulunmuyor.</p>";
            return;
        }
...
async function dinamikAnaKategorileriYukle() {
    ...
    try {
        ...
        const cevap = await fetch("categories.json");
        ...
        const mevcutlar = [
            ...
            const eslesme = href.match(/kat=([^&]+)/);

            return eslesme ? decodeURIComponent(eslesme[1]) : "";

        });

        kategoriler.forEach(k => {

            if (mevcutlar.includes(k.id)) return;

            const kart = document.createElement("a");
            kart.className = "category";
            kart.href = `kategori.html?kat=${k.id}`;

            kart.innerHTML = `
                <div class="icon">
                    <img src="./assets/icons/default.webp"
                         alt="${k.name}">
                </div>
                <span>${k.name}</span>
            `;
        ...
        console.error("Dinamik kategori yüklenemedi.", e);
